...
const MenuItem = ({ item, locale, priority = false }: { item: ProductWithRelations, locale: string, priority?: boolean }) => {
    ...
    return (
        <li className="group relative bg-card rounded-5xl p-6 transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-border/50 flex flex-col h-full overflow-hidden">
            {/* Background Glow */}
            ...
            <Link href={`/${locale}/menu/${item.id}`} className="relative flex-1">
                {/* Image Container */}
                <div className="relative w-full aspect-square mb-6 overflow-hidden rounded-4xl bg-secondary/30">
                    ...
                    <div className={`absolute top-4 inset-x-4 flex items-center justify-between pointer-events-none`}>
                        {/* Offer Badge */}
                        {item.onOffer && (
                            <div className="bg-linear-to-r from-primary to-orange-500 text-black px-3 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 shadow-xl animate-pulse scale-90 origin-left border border-white/20">
                                <Sparkles className="w-3.5 h-3.5" />
                                {isRtl ? 'عرض خاصة' : 'Offer'}
                            </div>
                        )}

                        {/* Quality Badge */}
                        <div className={`bg-background/90 backdrop-blur-md px-3 py-1 rounded-full shadow-sm flex items-center gap-1 border border-border/50 scale-90 group-hover:scale-100 transition-transform duration-500 ml-auto`}>
